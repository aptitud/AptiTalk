...
var addUser = function (user, callback) {
  if (user.identifier === "" || user.name === "" || user.email === "") {
    callback(createError("User must have an identifier, name and email"));
    return;
  }

  getUser(user.identifier, function (result) {
    if (result.success === false) {
      callback(result);
      return;
    }
    ...
    var usr = new User();
    usr.identifier = user.identifier;
    usr.name = user.name;
    usr.email = user.email;

    parseNickName(user.email, function (nicknameResult) {
      if (nicknameResult.success === false) {
        callback(nicknameResult);
        return;
      }

      usr.nickname = nicknameResult.data;

      User.create(usr, function (err, u) {
        if (err) {
