...
module.exports.addReply = function (id, username, message, xssIt, hashtagParser, callback) {
    getPostById(id, function (result) {
        ...
        internalAddPost(username, message, replyLevel, xssIt, hashtagParser, function (replyResult) {
            if (replyResult.success === false) {
                callback(replyResult);
                return;
            }

            var reply = replyResult.data;
            // console.log("Updating " + id + " with: " + reply.message);
            ...
            Post.update(
                ...
                function (err, updateResult) {
                    ...
                    // console.log("Update ok, modified: " + updateResult.n);
...
module.exports.getPostsForUser = function (username, callback) {
    Post
        .find({
            username: username
        })
        ...
        .exec(function (err, posts) {
            if(err) {
                callback(createError("Error getting username\n" + err));
                return;
            }
