...
var createPost = function (user, message, callback) {
  ...
  dbAccess.addPost(user.name, xssIt(message), hashTagParser.getHashTags, function (result) {
...
var replyPost = function (id, user, message, callback) {
  dbAccess.addReply(id, user.name, xssIt(message), hashTagParser.getHashTags, function (result) {
    if (result.success === false) {
      console.log(result.errorMessage);
      callback(result);
      return;
...
exports.connection = function (theIo, socket, theMode) { // First connection
  ...
  socket.on('getPosts', function () {
    getPosts(function (posts) {
      console.log('SERVER - loadPosts', posts);
